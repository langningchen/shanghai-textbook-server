...
export function getPublisherDisplayName(publisher: string): string {
	return (
		PUBLISHER_MAPPINGS[publisher as keyof typeof PUBLISHER_MAPPINGS] || "未知"
	);
}
	...
	return books.filter((book) => {
		...
		if (filters.search) {
			...
			const searchableText =
				`${book.title} ${book.short_title} ${book.subject_str}`.toLowerCase();
...
export function generateFriendlyFilename(book: Textbook): string {
	...
	const publisher = book.publisher
		? sanitize(getPublisherDisplayName(book.publisher))
		: "";
