			...
			<DialogContent sx={{ pt: 1 }}>
				{loading ? (
					...
					<Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
								...
								>
									...
									>
										...
										{book.is_current_term === 1 && (
											<Chip label="当前学期" size="small" color="success" />
										)}
									</Box>
						...
						{/* 使用年级学期 */}
						{book.used_grade_semesters &&
							book.used_grade_semesters.length > 0 && (
								<Paper sx={{ p: 2 }}>
									<Typography
										variant="h6"
										gutterBottom
									...
									{book.used_grade_semesters.map((gradeSemester, index) => (
										<Box key={index} sx={{ mb: 2 }}>
											...
											>
												{gradeSemester.grade_level_str} (
												{gradeSemester.grade_level_code})
											</Typography>
											<Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
												{gradeSemester.used_semester_phases.map(
													(phase, phaseIndex) => (
														<Chip
															key={phaseIndex}
